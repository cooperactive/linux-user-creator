...
cleanup() {
    [ -n "$PASSWORDS_FILE" ] && rm -f "$PASSWORDS_FILE"
}
trap cleanup EXIT
...
# Prompt for the corresponding account names
read -p "Enter the corresponding account names for the users (space-separated): " ACCOUNT_NAMES

# Convert the space-separated usernames and account names into arrays
USER_ARRAY=($USERS)
ACCOUNT_ARRAY=($ACCOUNT_NAMES)

# Check if the number of usernames matches the number of account names
if [ ${#USER_ARRAY[@]} -ne ${#ACCOUNT_ARRAY[@]} ]; then
    echo "Error: The number of usernames and account names must match."
    exit 1
fi
